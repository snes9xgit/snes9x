...
char rs_type_size = TEXT('b'), rs_last_type_size = rs_type_size;
...
void prune(char c,char o,char t,int v,int p)
{
	...
	switch (o) \
	{ \
		case TEXT('<'): DO_SEARCH_2(LessCmp,sf); break; \
		case TEXT('>'): DO_SEARCH_2(MoreCmp,sf); break; \
		case TEXT('='): DO_SEARCH_2(EqualCmp,sf); break; \
		case TEXT('!'): DO_SEARCH_2(UnequalCmp,sf); break; \
		case TEXT('l'): DO_SEARCH_2(LessEqualCmp,sf); break; \
		case TEXT('m'): DO_SEARCH_2(MoreEqualCmp,sf); break; \
		case TEXT('d'): DO_SEARCH_2(DiffByCmp,sf); break; \
		case TEXT('%'): DO_SEARCH_2(ModIsCmp,sf); break; \
...
int ReadControlInt(int controlID, bool forceHex, BOOL& success)
{
	...
	if(!forceHex)
	{
		rv = GetDlgItemInt(RamSearchHWnd,controlID,&ok,(rs_t == TEXT('s')));
	}
	...
	if(!ok)
	{
		...
		{
			for(int i = 0; Str_Tmp[i]; i++) {if(toupper(Str_Tmp[i]) == TEXT('O')) Str_Tmp[i] = TEXT('0');}
			const TCHAR* strPtr = Str_Tmp;
			bool negate = false;
			while(strPtr[0] == TEXT('-'))
				strPtr++, negate = !negate;
			if(strPtr[0] == TEXT('+'))
				strPtr++;
			if(strPtr[0] == TEXT('0') && tolower(strPtr[1]) == TEXT('x'))
				strPtr += 2, forceHex = true;
			if(strPtr[0] == TEXT('$'))
				strPtr++, forceHex = true;
			if(!forceHex)
			{
				const TCHAR* strSearchPtr = strPtr;
				while(*strSearchPtr)
				{
					int c = _totlower(*strSearchPtr++);
					if(c >= TEXT('a') && c <= TEXT('f'))
						forceHex = true;
				}
			}
			const TCHAR* formatString = forceHex ? TEXT("%X") : ((rs_t==TEXT('s')) ? TEXT("%d") : TEXT("%u"));
			if(_stscanf(strPtr, formatString, &rv) > 0)
				ok = true;
			if(negate)
				rv = -rv;
		}
...
bool Set_RS_Val()
{
	...
	switch(rs_c)
	{
		...
		default:
			...
			break;
		case TEXT('s'):
			rs_val = ReadControlInt(IDC_EDIT_COMPAREVALUE, rs_t == TEXT('h'), success);
			if(!success)
				return false;
			if((rs_type_size == TEXT('b') && rs_t == TEXT('s') && (rs_val < -128 || rs_val > 127)) ||
			   (rs_type_size == TEXT('b') && rs_t != TEXT('s') && (rs_val < 0 || rs_val > 255)) ||
			   (rs_type_size == TEXT('w') && rs_t == TEXT('s') && (rs_val < -32768 || rs_val > 32767)) ||
			   (rs_type_size == TEXT('w') && rs_t != TEXT('s') && (rs_val < 0 || rs_val > 65535)))
			   return false;
			break;
		case TEXT('a'):
	...
	// validate that rs_param fits in the comparison data type
	{
		...
		if(rs_c == TEXT('n'))
			appliedSize = TEXT('w'), appliedSign = TEXT('u');
		if(rs_c == TEXT('a'))
			appliedSize = TEXT('d'), appliedSign = TEXT('u');
		if((appliedSize == TEXT('b') && appliedSize == TEXT('s') && (rs_param < -128 || rs_param > 127)) ||
		   (appliedSize == TEXT('b') && appliedSize != TEXT('s') && (rs_param < 0 || rs_param > 255)) ||
		   (appliedSize == TEXT('w') && appliedSize == TEXT('s') && (rs_param < -32768 || rs_param > 32767)) ||
		   (appliedSize == TEXT('w') && appliedSize != TEXT('s') && (rs_param < 0 || rs_param > 65535)))
		   return false;
	}
...
bool IsSatisfied(int itemIndex)
{
	...
	{
		#undef DO_SEARCH_2
		#define DO_SEARCH_2(CmpFun,sf) return CALL_WITH_T_SIZE_TYPES_4(sf, rs_type_size,(rs_t==TEXT('s')),noMisalign, CmpFun,itemIndex,rs_val,rs_param);
		case TEXT('r'): DO_SEARCH(CompareRelativeAtItem); break;
		case TEXT('s'): DO_SEARCH(CompareSpecificAtItem); break;
		...
		case TEXT('a'): DO_SEARCH(CompareAddressAtItem); break;
		...
		case TEXT('n'): DO_SEARCH(CompareChangesAtItem); break;
	}
	return false;
}
...
void CompactAddrs()
{
	int size = (rs_type_size==TEXT('b') || !noMisalign) ? 1 : 2;
	int prevResultCount = ResultCount;

	CalculateItemIndices(size);
	ResultCount = CALL_WITH_T_SIZE_TYPES_0(CountRegionItemsT, rs_type_size,rs_t==TEXT('s'),noMisalign);
...
{
	EnterCriticalSection(&s_activeMemoryRegionsCS);
	CALL_WITH_T_SIZE_TYPES_0(UpdateRegionsT, rs_type_size, rs_t==TEXT('s'), noMisalign);
	LeaveCriticalSection(&s_activeMemoryRegionsCS);
}
...
void signal_new_size ()
{
	...
	int oldSize = (rs_last_type_size==TEXT('b') || !rs_last_no_misalign) ? 1 : 2;
	int newSize = (rs_type_size==TEXT('b') || !noMisalign) ? 1 : 2;
	...
	unsigned int oldTopAddr = CALL_WITH_T_SIZE_TYPES_1(GetHardwareAddressFromItemIndex, rs_last_type_size,rs_t==TEXT('s'),rs_last_no_misalign, oldTopIndex);
	unsigned int oldSelectionAddr = CALL_WITH_T_SIZE_TYPES_1(GetHardwareAddressFromItemIndex, rs_last_type_size,rs_t==TEXT('s'),rs_last_no_misalign, oldSelectionIndex);
...
//		Clear_Sound_Buffer();
		...
		int size = (rs_last_type_size==TEXT('b') || !rs_last_no_misalign) ? 1 : 2;
		int watchIndex = -1;
		for(int i = 0; i < selCount; ++i)
		{
			watchIndex = ListView_GetNextItem(lv, watchIndex, LVNI_SELECTED);
			int addr = CALL_WITH_T_SIZE_TYPES_1(GetHardwareAddressFromItemIndex, rs_last_type_size,rs_t==TEXT('s'),rs_last_no_misalign, watchIndex);
	...
	if(numberOfItemsChanged)
	{
		...
		unsigned int newTopIndex = CALL_WITH_T_SIZE_TYPES_1(HardwareAddressToItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, oldTopAddr);
		...
		for(unsigned int i = 0; i < selHardwareAddrs.size(); i++)
		{
			...
			int selRangeTop = CALL_WITH_T_SIZE_TYPES_1(HardwareAddressToItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, range.addr);
			int selRangeBottom = -1;
			for(int endAddr = range.End()-1; endAddr >= selRangeTop && selRangeBottom == -1; endAddr--)
				selRangeBottom = CALL_WITH_T_SIZE_TYPES_1(HardwareAddressToItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, endAddr);
			if(selRangeBottom == -1)
		...
		EnableWindow(GetDlgItem(RamSearchHWnd,IDC_MISALIGN), rs_type_size != TEXT('b'));
	}
	else
	{
...
//			Clear_Sound_Buffer();
	...
	else
	{
		...
		if (DoAutoSearch && ResultCount)
		{
			...
			if(rs_val_valid)
				prune(rs_c,rs_o,rs_t==TEXT('s'),rs_val,rs_param);
		}
	}
	...
	if(RamSearchHWnd)
	{
		...
		else
		{
			...
			{
				int changeNum = CALL_WITH_T_SIZE_TYPES_1(GetNumChangesFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, i); //s_numChanges[i];
...
LRESULT CALLBACK RamSearchProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
		case WM_INITDIALOG: {
			...
			//const TCHAR* names[5] = {TEXT("Address"),TEXT("Value"),TEXT("Previous"),TEXT("Changes"),TEXT("Notes")};
			//int widths[5] = {62,64,64,55,55};
			const TCHAR* names[] = {TEXT("Address"),TEXT("Value"),TEXT("Previous"),TEXT("Changes")};
			...
			void init_list_box(HWND Box, const TCHAR* Strs[], int numColumns, int *columnWidths);
				...
				case LVN_GETDISPINFO:
				{
					...
					if (Item->item.mask & LVIF_TEXT)
					{
						...
						switch (Item->item.iSubItem)
						{
							case 0:
							{
								HWAddressType addr = CALL_WITH_T_SIZE_TYPES_1(GetHardwareAddressFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, iNum);
								lstrcpy(num, RWInternalToDisplayAddress(addr));
								Item->item.pszText = num;
							}	break;

							case 1:
							{
								int i = CALL_WITH_T_SIZE_TYPES_1(GetCurValueFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, iNum);
								const TCHAR* formatString = ((rs_t==TEXT('s')) ? TEXT("%d") : (rs_t==TEXT('u')) ? TEXT("%u") : (rs_type_size==TEXT('d') ? TEXT("%08X") : rs_type_size==TEXT('w') ? TEXT("%04X") : TEXT("%02X")));
								switch (rs_type_size)
								{
									case TEXT('b'):
									default: _stprintf(num, formatString, rs_t==TEXT('s') ? (char)(i&0xff) : (unsigned char)(i&0xff)); break;
									case TEXT('w'): _stprintf(num, formatString, rs_t==TEXT('s') ? (short)(i&0xffff) : (unsigned short)(i&0xffff)); break;
									case TEXT('d'): _stprintf(num, formatString, rs_t==TEXT('s') ? (long)(i&0xffffffff) : (unsigned long)(i&0xffffffff)); break;
								}
								Item->item.pszText = num;
							}	break;
							...
							case 2:
							{
								int i = CALL_WITH_T_SIZE_TYPES_1(GetPrevValueFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, iNum);
								const TCHAR* formatString = ((rs_t==TEXT('s')) ? TEXT("%d") : (rs_t==TEXT('u')) ? TEXT("%u") : (rs_type_size==TEXT('d') ? TEXT("%08X") : rs_type_size==TEXT('w') ? TEXT("%04X") : TEXT("%02X")));
								switch (rs_type_size)
								{
									case TEXT('b'):
							...
							case 3:
							{
								int i = CALL_WITH_T_SIZE_TYPES_1(GetNumChangesFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, iNum);
								_stprintf(num,TEXT("%d"),i);
...
//					{rv = true; break;}				
				...
				case IDC_C_ADDCHEAT:
				{
					...
					{
						unsigned long address = CALL_WITH_T_SIZE_TYPES_1(GetHardwareAddressFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, watchItemIndex);

						int sizeType = -1;
						if(rs_type_size == TEXT('b'))
							sizeType = 0;
						else if(rs_type_size == TEXT('w'))
							sizeType = 1;
						else if(rs_type_size == TEXT('d'))
							sizeType = 2;
						...
						cht.new_val = CALL_WITH_T_SIZE_TYPES_1(GetCurValueFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, watchItemIndex);
...
//					Clear_Sound_Buffer();
					...
					if(!ResultCount)
					{
						...
						MessageBox(RamSearchHWnd,TEXT("Resetting search."),TEXT("Out of results."),MB_OK|MB_ICONINFORMATION);
						soft_reset_address_info();
					}
...
invalid_field:
					MessageBox(RamSearchHWnd,TEXT("Invalid or out-of-bound entered value."),TEXT("Error"),MB_OK|MB_ICONSTOP);
				...
				case IDC_C_WATCH:
				{
					...
					while (watchItemIndex >= 0)
					{
						...
						tempWatch.Address = CALL_WITH_T_SIZE_TYPES_1(GetHardwareAddressFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, watchItemIndex);
						...
						if (selCount == 1)
							...
							inserted |= InsertWatch(tempWatch, TEXT(""));
				...
				case IDC_C_ELIMINATE:
				{
					...
					for(int i = 0, j = 1024; i < selCount; ++i, --j)
					{
						...
						int addr = CALL_WITH_T_SIZE_TYPES_1(GetHardwareAddressFromItemIndex, rs_type_size,rs_t==TEXT('s'),noMisalign, watchIndex);
						if(!selHardwareAddrs.empty() && addr == selHardwareAddrs.back().End())
							selHardwareAddrs.back().size += size;
						else
							selHardwareAddrs.push_back(AddrRange(addr,size));
...
{
#define HEADER_STR TEXT(" RAM Search - ")
#define PROGRESS_STR TEXT(" %d%% ... ")
#define STATUS_STR TEXT("%d Possibilit%s (%d Region%s)")

	int poss = last_rs_possible;
	int regions = last_rs_regions;
	if(poss <= 0)
		lstrcpy(Str_Tmp,TEXT(" RAM Search"));
	else if(percent <= 0)
		_stprintf(Str_Tmp, HEADER_STR STATUS_STR, poss, poss==1?TEXT("y"):TEXT("ies"), regions, regions==1?TEXT(""):TEXT("s"));
	else
		_stprintf(Str_Tmp, PROGRESS_STR STATUS_STR, percent, poss, poss==1?TEXT("y"):TEXT("ies"), regions, regions==1?TEXT(""):TEXT("s"));
...
void SetRamSearchUndoType(HWND hDlg, int type)
{
	if(s_undoType != type)
	{
		if((s_undoType!=2 && s_undoType!=-1)!=(type!=2 && type!=-1))
			SendDlgItemMessage(hDlg,IDC_C_UNDO,WM_SETTEXT,0,(LPARAM)((type == 2 || type == -1) ? TEXT("Redo") : TEXT("Undo")));
