...
//Sound Settings
#define SNDSET_VOLUME_REGULAR TEXT("Adjust the volume of the playback.")
#define SNDSET_VOLUME_TURBO TEXT("Adjust the volume of the playback that occurs when fast-forwarding the program.")
#define SNDSET_OUTPUT_DEVICE TEXT("Set the device through which to output sound.")
#define SNDSET_RATE TEXT("Adjust the sample rate for output. Higher rates generally result in an improved sound quality.")
#define SNDSET_INRATE TEXT("For each 'Input rate' samples generated by the SNES, 'Playback rate' samples will produced. If you experience crackling you can try to lower this setting.")
#define SNDSET_DYNRATECONTROL TEXT("Try to dynamically adjust the input rate to never overflow or underflow the sound buffer.")

//Display Settings

#define DSPSET_OUTPUTMETHOD TEXT("Select the method by which to display output")
#define DSPSET_FULLSCREEN TEXT("Toggle between fullscreen and windowed mode")
#define DSPSET_FULLSCREEN_ON_OPEN TEXT("Switch to fullscreen on loading a ROM file")
#define DSPSET_EMUFULLSCREEN TEXT("Emulate fullscreen by creating a window that spans the entire screen when going fullscreen")
#define DSPSET_STRETCH TEXT("Scale the image to window")
#define DSPSET_AUTOFRAME TEXT("Try to achieve 50/60 fps by limiting the speed and skipping at most 'max skipped frames'")
#define DSPSET_FIXEDSKIP TEXT("Always skip a fixed number of frames - no speed limit")
#define DSPSET_HIRES TEXT("Support the hi-res mode that a few games use, otherwise render them in low-res")
#define DSPSET_HEIGHT_EXTEND TEXT("Display an extra 15 pixels at the bottom, which few games use. Also increases AVI output size from 256x224 to 256x240")
#define DSPSET_MESSAGES_IN_IMAGE TEXT("Draw text inside the SNES image (will get into AVIs, screenshots, and filters)")
#define DSPSET_MESSAGES_SCALE TEXT("Try to scale messages with EPX instead of Simple, only works for 2x and 3x and when displaying after filters")
